...
// *返回: 秒
// *========================================
inline int task_convert_time(char **str_array) {
    int sec = 0;
    int cnt = atoi(str_array[0]);

    for (int num = 1; num <= cnt; num++) {
        switch (num) {
        case 1: //sec
            sec += atoi(str_array[num]);
            break;
        case 2: //min
            sec += atoi(str_array[num]) * 60;
            break;
        case 3: //hour
            sec += atoi(str_array[num]) * 60 * 60;
            break;
        case 4: //day
            sec += atoi(str_array[num]) * 24 * 60 * 60;
        }
    }
    return sec > 0 ? sec : 0;
}
        ...
        enum mjson_type type)
{
    ...
    if (!strcmp(key, SYSKEY_GLOBAL_LIMIT_RUN_CNT_UP_TO) && type == mjson_type_int) {
        ...
        DEBUG(DATA, "limit: tinatest_run_time_sec : %d => %d\n",
                ...
                task_convert_time(val.m_array));
        genv->limit.tinatest_run_time_sec = task_convert_time(val.m_array);
    } else {
        return 0;
    }
        ...
        enum mjson_type type)
{
    ...
    //limit
        ...
        DEBUG(DATA, "limit: testcase_run_once_time_sec: %d => %d\n",
                ...
                task_convert_time(val.m_array));
        lenv->limit.testcase_run_once_time_sec = task_convert_time(val.m_array);
    }
        ...
        DEBUG(DATA, "limit: testcase_run_time_sec: %d => %d\n",
                ...
                task_convert_time(val.m_array));
        lenv->limit.testcase_run_time_sec = task_convert_time(val.m_array);
    }
    else if (!strcmp(key, SYSKEY_LOCAL_LIMIT_TIMEOUT_WITH_PASS)
            && type == mjson_type_boolean) {
        DEBUG(DATA, "limit: timeout_with: %d => %d\n",
                lenv->limit.timeout_with, val.m_boolean == true ? TASK_RESULT_PASS : SIGALRM);
        lenv->limit.timeout_with = val.m_boolean == true ? TASK_RESULT_PASS : SIGALRM;
    }
    else if (!strcmp(key, SYSKEY_LOCAL_LIMIT_TIMEOUT_WITH_FAILED)
            && type == mjson_type_boolean) {
        DEBUG(DATA, "limit: timeout_with: %d => %d\n",
                lenv->limit.timeout_with, val.m_boolean == true ? SIGALRM : TASK_RESULT_PASS);
        lenv->limit.timeout_with = val.m_boolean == true ? SIGALRM : TASK_RESULT_PASS;
    }
...
int task_add_do(const char *keypath, const char *command)
{
    ...
    while ((p = strchr(p, '/')) != NULL)
        *p = '-';
    new->logpath = calloc(1, max(len, sizeof(REBOOT_LOG)) + 1);
    if (new->logpath == NULL) {
        ERROR("%s: malloc failed - %s\n", keypath, strerror(errno));
        goto out1;
    }
    ...
    snprintf(buf, len * 2, "%s/%s", new->keypath, SYSKEY_TASK_STDIN);
    if ((pp = mjson_fetch_array(buf)) != NULL) {
        new->input = pp;
        new->input_t = TASK_STDIN;
    } else {
        snprintf(buf, len * 2, "%s/%s", new->keypath, SYSKEY_TASK_FSTDIN);
...
void task_recycle_once(struct task *task, int num, int status, int pid, struct rusage *res)
{
    ...
    if (WIFEXITED(status))
        ...
        if (task->killed[num] == SIGALRM)
            task->result[num] = task->env.limit.timeout_with;
        else
