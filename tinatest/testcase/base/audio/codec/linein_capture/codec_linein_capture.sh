...
keypath="/base/audio/codec/linein_capture"
ucm_params="-c audiocodec -v Record -d LineIn"
...
ttrue "Is the line in ready?"
if [ $? -ne "0" ]; then
    ttips "ERROR: line in is not ready"
    exit 1
fi
...
if [ $? -ne "0" ]; then
    ttips "Failed to enable the UCM device \"LineIn\""
    continue
fi
pcm_device=$(alsa-ucm-aw $ucm_params get CapturePCM)
if [ $? -ne "0" ]; then
    ttips "Failed to get the \"CapturePCM\" of \"LineIn\""
    continue
fi
record="${records_dir}/record_${i}.wav"
ttips "Start recording with line in"
...
ttips "Finish playing the record"
...
ttrue "Can you hear the record of line in?"
if [ $? -eq "0" ]; then
    exit 0
else
    exit 1
