...
int main(int argc, char **argv)
{
	...
	test_info.screen_id = mjson_fetch_int(
			"/base/production/displaytester/capturetester/screen_id");
	...
	test_info.layer_id = mjson_fetch_int(
			"/base/production/displaytester/capturetester/layer_id");
	...
	test_info.channel_id = mjson_fetch_int(
			"/base/production/displaytester/capturetester/channel_id");
	...
	test_info.layer_num = mjson_fetch_int(
			"/base/production/displaytester/capturetester/layer_num");
	...
	for (i = 0; i < fb_num; i++) {
		fp = fopen("/usr/share/capture01.bmp", "wb+");
		if (fp < 0) {
	...
	close(test_info.dispfh);
	printf("GOOD, The image is saved in /usr/share/capture01.bmp\n");
