...
void parse_configs(void)
{
	fb_id = mjson_fetch_int(
			"/base/production/displaytester/fbshottester/fb_id");
	if (fb_id != -1) {
...
int main(int argc, char *argv[])
{
	...
	img_fd = open("/usr/share/fbshottester.bmp", O_RDWR | O_CREAT, 0644);
	if (img_fd < 0) {
		perror("open image");
		close(img_fd);
		return -1;
	...
	printf("GOOD, The image is saved in /usr/share/fbshottester.bmp\n");
